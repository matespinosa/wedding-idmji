...
export function Gallery() {
  ...
  /* ————————————————————————————————————————————————
     Empujón de cortesía. Al entrar en vista, el álbum se desplaza un
     poco y regresa: se ve cómo las fotos corren hacia la izquierda y
     asoma la siguiente, que es la forma más directa de decir «esto se
     desliza». Se cancela en cuanto la persona toca el carrusel, y no
     corre con reduced-motion ni si no hay nada que recorrer.
     ———————————————————————————————————————————————— */
  useEffect(() => {
    const track = trackRef.current;
    if (!track) {
    ...
    for (const event of events) {
      track.addEventListener(event, cancel, { passive: true });
    }
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        /* Lo justo para descubrir el borde de la foto siguiente. */
        const reach = Math.min(96, max, track.clientWidth * 0.18);
        const cursor = { x: track.scrollLeft };
        const apply = () => {
          track.scrollLeft = cursor.x;
        };

        timeline = gsap
          .timeline({ delay: 0.6, repeat: 1, repeatDelay: 0.6 })
          .to(cursor, {
            x: reach,
            duration: 0.72,
            ease: "power2.inOut",
          ...
          .to(
            cursor,
            { x: 0, duration: 0.86, ease: "power2.inOut", onUpdate: apply },
            "+=0.24",
          );
      },
      { threshold: 0.4 },
    ...
    return () => {
      observer.disconnect();
      for (const event of events) {
        track.removeEventListener(event, cancel);
      }
      timeline?.kill();
    };
  }, []);
    ...
    >
      ...
      <div className="relative mx-auto mt-8 max-w-6xl px-5 md:px-8">
        ...
        <p
          ...
          className="mt-5 text-center text-[10px] font-medium uppercase tracking-[0.26em] text-cream/45"
        >
          {site.gallery.hint}
        </p>
      </div>
